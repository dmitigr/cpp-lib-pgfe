...
struct Generic_string_conversions final {
  ...
  static Type to_type(const std::string& text, Types&& ...)
  {
    ...
    stream >> result;
    if (!stream.eof())
      throw Generic_exception{"cannot convert to type: "
  ...
  static std::string to_string(const Type& value, Types&& ...)
  {
    ...
    stream << value;
    if (stream.fail())
      throw Generic_exception{"cannot convert to string: "
...
protected:
    ...
    R(*converter)(const std::string&, std::size_t*, Types ...))
  {
    ...
    if (idx != text.size())
      throw Generic_exception{"cannot convert to numeric: "
...
struct Char_string_conversions final {
  ...
  {
    if (!(text.size() == 1))
      throw Generic_exception{"cannot convert to char: invalid input size"};
    return text[0];
  }
...
private:
  ...
  static Type to_type__(const char* const text, const std::size_t size)
  {
    if (!text)
      ...
      throw Generic_exception{"cannot convert to bool: "
        "invalid text representation"};
  }
};
