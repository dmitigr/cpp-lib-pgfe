...
public:
  ...
  {
#ifdef DMITIGR_PGFE_AIO
    const auto* const result = static_cast<const Connection*>(this)->loop();
    return const_cast<std::decay_t<decltype(result)>>(result);
#else
    return nullptr;
#endif
  }
   ...
   */
  DMITIGR_PGFE_API Status connect_nio();

#ifdef DMITIGR_PGFE_AIO
  DMITIGR_PGFE_API void connect_aio(Aio_handler,
   ...
   * value of `std::nullopt` means *eternity*.
   *
   * @par Requires
   ...
   *
   * @par Requires
   * Similar to wait_socket_readiness().
   *
   ...
   *
   * @par Requires
   * `is_nio_output_enabled()`.
   *
   * @par Effects
   * is_output_flushed() returns a value returned by this function.
   *
   * @see set_nio_output_enabled().
   */
  ...
  /**
   * @brief Similar to wait_response(), but throws Sqlstate_exception if `error()`
   * after awaiting.
   *
   * @see wait_response().
   */
      ...
      try {
        ...
        if constexpr (on_exception == Row_processing::complete) {
          ...
          try {
            ...
            try {
              process_responses_error = Generic_exception{e.what()};
            } catch (...) {
              goto bad_alloc;
            }
          } catch (...) {}
          ...
          if (comp)
            ...
            std::throw_with_nested(Sqlstate_exception{std::move(e)});
...
#ifdef DMITIGR_PGFE_AIO
  DMITIGR_PGFE_API void prepare_aio(Aio_handler handler,
    const Statement& statement, const std::string& name = {});

  DMITIGR_PGFE_API void describe_aio(Aio_handler handler,
    const std::string& name);
#endif
  ...
  execute(F&& callback, const Statement& statement, Types&& ... parameters)
  {
    if (!is_ready_for_request())
      throw Generic_exception{"cannot execute statement: not ready for request"};
...
private:
...
#ifdef DMITIGR_PGFE_AIO
    Aio_handler response_handler_;
#endif
  };

  Connection(void* loop, Options options);
...
#ifdef DMITIGR_LIBS_AIO_ASIO
  using Asio_ip_tcp = DMITIGR_LIBS_AIO_ASIO_NAMESPACE::ip::tcp;
  DMITIGR_LIBS_AIO_ASIO_NAMESPACE::io_context* loop_{};
  std::optional<Asio_ip_tcp::socket> loop_sock_;
  auto& loop_sock() noexcept
  {
    DMITIGR_ASSERT(loop_sock_);
    return *loop_sock_;
  }

  void aio_handle(const Err&, const Request&) noexcept;
  void aio_disconnect_and_handle(const Err&) noexcept;

  template<typename F>
  auto asio_handler(F&& f) noexcept
  {
    return [this, f = std::move(f)](const std::error_code& ec)
    {
      try {
        if (ec)
          throw dmitigr::Exception{ec.default_error_condition()};
        else
          f();
  ...
  void aio_set_establishment_handler(Asio_ip_tcp::socket::wait_type);
  void aio_set_connected_read_ready_handler();
  void aio_set_connected_write_ready_handler();
  void aio_flush_output()
  {
    flush_output(false);
    aio_set_connected_write_ready_handler();
  }

  Aio_handler connect_handler_;
#endif
  Options options_;
  ...
  Response_status response_status_{Response_status::empty}; // ... by handle_input()
  ...
  {
    if (function.empty())
      throw Generic_exception{"cannot call/invoke: routine not specified"};
