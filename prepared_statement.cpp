...
Prepared_statement::execute_nio__(const Statement* const statement)
{
  ...
  try {
    ...
    if (!send_ok)
      throw Generic_exception{conn.error_message()};
...
DMITIGR_PGFE_INLINE void Prepared_statement::execute_aio(Aio_handler handler)
{
  execute_nio();
  auto& conn = connection();
  DMITIGR_ASSERT(!conn.requests_.empty());
  conn.requests_.front().response_handler_ = std::move(handler);
  conn.aio_flush_output();
}
#endif
